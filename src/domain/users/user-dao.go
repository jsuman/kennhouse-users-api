...
func (user *User) FindByEmail() *errors.RestErr {
	stmt, err := usersdb.Client.Prepare("select id, first_name, last_name, email, date_created, password, status from users_db.users where email=? and status=?;")
	...
	if err := result.Scan(&user.Id, &user.FirstName, &user.LastName, &user.Email, &user.DateCreated, &user.Password, &user.Status); err != nil {
		...
		logger.Error("error while trying to get the user by email and password", err)
		return errors.InternalServerError("database error")
	}
